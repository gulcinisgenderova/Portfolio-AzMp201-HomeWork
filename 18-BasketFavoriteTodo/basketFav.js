...
function creatTo(product) {
    ...
    prodPrice.textContent = `Price: $${product.price}`;
    prodRating.textContent = `Rating: ${product.rating.rate} (${product.rating.count} reviews)`;
    ...
    item.append(itemImg, prodTitle, prodPrice, prodRating, basketButton, favButton);
    homePage.appendChild(item);

    return item;
}

function addToBasket(api) {
    basketArr.push(api);
    renderBasket();
    function renderBasket() {
        basketPage.innerHTML = '';

        basketArr.forEach(api => {
            const item = document.createElement("div");
            const prodTitle = document.createElement("h5");
            const prodPrice = document.createElement("p");
